...
def get_server(server_url='http://localhost:5984/'):
...
def get_db(db_name, server_url='http://localhost:5984/'):
    """Return a CouchDB database instance, given its name."""
    return get_server(server_url)[db_name]

def get_doc(doc_id, db_name, server_url='http://localhost:5984/', rev=None):
